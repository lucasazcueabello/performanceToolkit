...
def getElementsFromCSV(file, metric, scope):
    ...
    with open(file_path, 'r', newline='') as csvfile:
        ...
        for row in reversed(csv_reader):
            if not isFileIncluded(row[0], data) and float(row[index]) > 0.0:
                data.append((row[0], float(row[index])))
            if len(data) >= len(readYAML()["data_structures"]):
                return data
    return data
...
def representComparison(file, metric, scope):
    ...
    plotBarGraphic(files, measurements, metric, "Segments", "comparison between data structures", metric + "_data_structures_comparison.png")


def main():
    if len(sys.argv) != 4:
        print("Invalid Arguments for the Data Structures Comparison module")
        return

    if sys.argv[1] == "segment":
        representComparison(removeCopyPrefix(sys.argv[3]), sys.argv[2], sys.argv[1])
